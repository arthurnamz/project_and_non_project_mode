/******************************************************************************
* Copyright (C) 2023 Advanced Micro Devices, Inc. All Rights Reserved.
* SPDX-License-Identifier: MIT
******************************************************************************/
/*
 * helloworld.c: simple test application
 *
 * This application configures UART 16550 to baud rate 9600.
 * PS7 UART (Zynq) is not initialized by this application, since
 * bootrom/bsp configures it to baud rate 115200
 *
 * ------------------------------------------------
 * | UART TYPE   BAUD RATE                        |
 * ------------------------------------------------
 *   uartns550   9600
 *   uartlite    Configurable only in HW design
 *   ps7_uart    115200 (configured by bootrom/bsp)
 */

#include <stdio.h>
#include "platform.h"
#include "xil_printf.h"
#include "xparameters"
#include <xil_io.h>
#include <stdlib.h>
#include <time.h>
#include "./../../platform_new/hw/sdt/drivers/simpleCpu_v1_0/src/simpleCpu.h"
#include "./../../platform_new/hw/sdt/drivers/mysimple_multi_v1_0/src/mysimple_multi.h"
#include "xtmrctr.h"


#define ROW 2
#define COL 2

#define TIMER_DEVICE_ID XPAR_AXI_TIMER_0_BASEADDR
#define TIMER_COUNTER_0 0

// A[m,p], B[p,m], C[m,m]

void genMatrix(u32* A){
    srand( time(NULL));
    for(int i = 0; i<(ROW*COL); ++i){
        A[i] = rand() % 10;
       // xil_printf("%d = %d",i,A[i]);
    }
   // xil_printf("\n");
}

void matMul(u32* A, u32* B, u32* C){
   // xil_printf("\nMUlt:\n");
    for(int i=0; i<ROW;i++){
        for(int j=0; j<COL;j++){
            u32 sum = 0;
            for(int k=0;k<COL;k++){
                sum = sum + A[i*COL+k] * B[k*COL+j];
            }
            C[i*COL+j] = sum;// C[i,j] = sum
           // xil_printf("%d,%d = %d\n",i,j, C[i*COL+j]);
        }
    }
}

void matMul_simpleCpu(u32* A, u32* B, u32* C){
    //xil_printf("\nMUlt:\n");
    for(int i=0; i<ROW;i++){
        for(int j=0; j<COL;j++){
            u32 sum = 0;
            for(int k=0;k<COL;k++){
               u32 tmp = A[i*COL+k] * B[k*COL+j];
                SIMPLECPU_mWriteReg(XPAR_SIMPLECPU_0_BASEADDR, SIMPLECPU_S00_AXI_SLV_REG0_OFFSET, sum);
                SIMPLECPU_mWriteReg(XPAR_SIMPLECPU_0_BASEADDR, SIMPLECPU_S00_AXI_SLV_REG1_OFFSET, tmp);
                sum = SIMPLECPU_mReadReg(XPAR_SIMPLECPU_0_BASEADDR, SIMPLECPU_S00_AXI_SLV_REG3_OFFSET);
            }
            C[i*COL+j] = sum;// C[i,j] = sum
           // xil_printf("%d,%d = %d\n",i,j, C[i*COL+j]);
        }
    }
}

u8 compareMatrices(u32* vec1, u32* vec2){

    for(int i=0;i<ROW*COL;++i){
        if(vec1[i] != vec2[i])
            return 0;
        return 1;
    }
}



int main()
{
    init_platform();

/*
    int i;
    u32 a,b,c;
    a = 5;
    b = 7;
    SIMPLECPU_mWriteReg(XPAR_SIMPLECPU_0_BASEADDR, SIMPLECPU_S00_AXI_SLV_REG0_OFFSET, a);
    SIMPLECPU_mWriteReg(XPAR_SIMPLECPU_0_BASEADDR, SIMPLECPU_S00_AXI_SLV_REG1_OFFSET, b);
    //c=SIMPLECPU_mReadReg(XPAR_SIMPLECPU_0_BASEADDR, SIMPLECPU_S00_AXI_SLV_REG3_OFFSET);
    c=Xil_In32(XPAR_SIMPLECPU_0_BASEADDR+SIMPLECPU_S00_AXI_SLV_REG3_OFFSET);

    xil_printf("c=%d", c);

    for(i=5;i<8;i=i+1)
        xil_printf( "%d\n", i);

    print("Hello World\n\r");
    // print("Successfully ran Hello World application");
    */

    //Matrix Space Allocation
    u32* A = malloc(ROW*COL*sizeof(u32));
    u32* B = malloc(ROW*COL*sizeof(u32));
    u32* C = malloc(ROW*COL*sizeof(u32));
    u32* D = malloc(ROW*COL*sizeof(u32));


    genMatrix(A);
    genMatrix(B);

    XTmrCtr InstancePtr;

    // 1. Inialize the AXI Timer IP
    int status = XTmrCtr_Initialize(&InstancePtr, TIMER_DEVICE_ID);
    if(status != XST_SUCCESS)
        xil_printf("fail\n");

    // 2. Set AXI Timer options (Auto-reload and counting up)
    XTmrCtr_SetOptions(&InstancePtr, TIMER_COUNTER_0, XTC_AUTO_RELOAD_OPTION);

    //3. Start and calibrate the counter
    XTmrCtr_Start(&InstancePtr, TIMER_COUNTER_0);

    u32 StartTime = XTmrCtr_GetValue(&InstancePtr, TIMER_COUNTER_0);
    u32 EndTime = XTmrCtr_GetValue(&InstancePtr, TIMER_COUNTER_0);

    u32 Calibration = EndTime - StartTime;
   // xil_printf("%d\n", Calibration);

    StartTime = 0;
    EndTime = 0;

    StartTime = XTmrCtr_GetValue(&InstancePtr, TIMER_COUNTER_0);
    matMul(A,B,C);
    EndTime = XTmrCtr_GetValue(&InstancePtr, TIMER_COUNTER_0);
    u32 Duration = EndTime - StartTime - Calibration;
    xil_printf(" %d \n", (Duration*10));

    StartTime = 0;
    EndTime = 0;
    Duration = 0;

    StartTime = XTmrCtr_GetValue(&InstancePtr, TIMER_COUNTER_0);
    matMul_simpleCpu(A,B, D);
    EndTime = XTmrCtr_GetValue(&InstancePtr, TIMER_COUNTER_0);
    Duration = EndTime - StartTime - Calibration;
    xil_printf(" %d \n", (Duration*10));
    //xil_printf("RES=%d\n", compareMatrices(C,D) );

    cleanup_platform();
    return 0;
}
